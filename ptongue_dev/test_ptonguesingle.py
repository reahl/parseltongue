...
from ptongue.gemproxysinglethread import LinkedSession as Session
...
@pytest.fixture
def invalid_session(stone_fixture):
    session = Session('DataCurator', 'swordfish')
    session.log_out()
    yield session
...
def test_exceptions_when_translating_wrong_gemstone_type(session):
    ...
    with expected(GemstoneError, test='The given object is not a float.'):
